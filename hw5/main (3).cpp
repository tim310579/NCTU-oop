...
int main()
{
    ...
    */
    ...
    int repeat = 0;
    bool ret;
	int key[5000001],gender[5000001],height[5000001],weight[5000001];
	string sex[5000001];
    for (int i = 0; i < 5000000; i++)
    {
        ...
        key[i] = rand31() % 100000000 + 1;
        gender[i] = rand() % 2;
        height[i] = 150 + rand() % 30;
        weight[i] = 40 + rand() % 50;
        sex[i] = (gender[i] == 0) ? "female" : "male";
		for(int j =0; j <=i;j++){
			if(key[j] == key[i]&& sex[j] == sex[i] &&height[j]==height[i] &&weight[j]==weight[i]){
				cout <<j<<" : "<<key[j]<<" "<< sex[j] << " "<< height[j] <<" "<<weight[j]<<endl;
				cout <<key[i]<<" "<< sex[i] << " "<< height[i] <<" "<<weight[i]<<endl;
			}
		}
		

	// ret = rbTree.insert(key[i], sex[i], height[i], weight[i]);
        // insert
